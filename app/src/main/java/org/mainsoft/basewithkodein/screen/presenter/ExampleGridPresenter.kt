...
class ExampleGridPresenter(view: ExampleGridView) : BaseListPresenter<String>(view) {
    ...
    override fun loadCache(savedInstanceState: Bundle) {
        onLoad(savedInstanceState.getStringArrayList(BUNDLE_LIST_KEY)?.toMutableList()
                ?: mutableListOf())
    }

    override fun saveCache(outState: Bundle) {
        outState.putStringArrayList(BUNDLE_LIST_KEY, ArrayList(getData()))
    }
